...
// Task represents a row from 'go_crud_sample.task'.
type Task struct {
	ID        uint      `json:"id"`         // id
...
func (t *Task) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO go_crud_sample.task (` +
...
func (t *Task) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE go_crud_sample.task SET ` +
...
func (t *Task) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM go_crud_sample.task WHERE id = ?`
...
// TaskByID retrieves a row from 'go_crud_sample.task' as a Task.
//
